...
export default function ReportsPage() {
  ...
  const ClassifiedSection = ({ title, accounts, balancesMap, className }: { title: string, accounts: COAEntry[], balancesMap: Record<string, number>, className?: string }) => {
    type Group = { header: COAEntry; items: COAEntry[]; total: number };
    const groups: Group[] = [];
        ...
        <div className="border-4 border-black overflow-hidden shadow-2xl">
          <table className="w-full text-xs border-collapse font-black text-black tabular-nums">
            ...
            <tfoot className="bg-black text-white border-t-4 border-black font-black">
              <tr className="h-16">
                <td colSpan={2} className="p-4 text-right uppercase tracking-[0.3em] text-sm pr-10">Grand Totals:</td>
  ...
  if (isEntriesLoading) return <div className="flex h-screen items-center justify-center bg-white"><Loader2 className="animate-spin size-12 text-black" /></div>;
    ...
    <div className="p-8 flex flex-col gap-8 bg-white min-h-screen font-ledger text-black">
      ...
      <div className="flex items-center justify-between no-print max-w-5xl mx-auto w-full bg-white p-6 rounded-3xl border-2 border-black shadow-2xl">
        <div className="flex items-center gap-5">
          ...
          <div className="flex flex-col">
            <h1 className="text-2xl font-black tracking-tighter text-black uppercase">Financials</h1>
      ...
      <Tabs defaultValue="position" className="w-full max-w-5xl mx-auto">
        <TabsList className="grid w-full grid-cols-4 mb-8 no-print h-14 bg-white border-2 border-black p-1.5 rounded-[20px] shadow-md">
          ...
          <TabsTrigger value="income" className="gap-2 rounded-xl text-sm font-black transition-all data-[state=active]:bg-black data-[state=active]:text-white uppercase"><TrendingUp className="size-4" /> Income Statement</TabsTrigger>
          ...
          <TabsTrigger value="receipts" className="gap-2 rounded-xl text-sm font-black transition-all data-[state=active]:bg-black data-[state=active]:text-white uppercase"><ArrowDownUp className="size-4" /> Cash Flow Statement</TabsTrigger>
        </TabsList>
          ...
          <Card className="border-2 border-black shadow-2xl rounded-none bg-white print-container print-portrait-fix mx-auto overflow-hidden">
            <CardContent className="p-12 print:p-0">
              ...
              <div className="space-y-12">
                <ClassifiedSection title="Assets & Portfolio" accounts={assetAccounts} balancesMap={balances} />
        ...
        <TabsContent value="income">
          <Card className="border-2 border-black shadow-2xl rounded-none bg-white print-container print-portrait-fix mx-auto overflow-hidden">
            <CardContent className="p-12 print:p-0">
              <ReportHeader title="Statement of Profit or Loss and Other Comprehensive Income" subtitle={`For the Year Ended June 30, ${fyDates.end.split('-')[0]}`} />
              <div className="space-y-12">
                <div className="space-y-6">
                  <h3 className="text-base font-black border-b-4 border-black text-black pb-1.5 uppercase tracking-[0.2em]">Revenue</h3>
        ...
        <TabsContent value="trial">
          <Card className="border-2 border-black shadow-2xl rounded-none bg-white print-container print-portrait-fix mx-auto overflow-hidden">
            <CardContent className="p-12 print:p-0">
              <ReportHeader title="Trial Balance" subtitle={`As of June 30, ${fyDates.end.split('-')[0]}`} />
              <TrialBalanceView />
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="receipts">
           <Card className="border-2 border-black shadow-2xl rounded-none bg-white print-container print-portrait-fix mx-auto overflow-hidden">
            <CardContent className="p-12 print:p-0">
              <ReportHeader title="Statement of Cash Flows" subtitle={`For the Year Ended June 30, ${fyDates.end.split('-')[0]}`} />
              <div className="grid grid-cols-1 gap-y-12 mt-8">
                 <div className="space-y-8">
                    <h4 className="font-black text-base text-black border-b-4 border-black pb-1.5 uppercase tracking-[0.2em]">Cash Inflows</h4>
                 ...
                 <div className="space-y-8 border-t-4 border-black pt-10">
                    <h4 className="font-black text-base text-black border-b-4 border-black pb-1.5 uppercase tracking-[0.2em]">Cash Outflows</h4>
                    <div className="space-y-2">
                 ...
                 <div className="flex justify-between font-black text-xl p-6 border-[4px] border-black rounded-2xl bg-slate-50 text-black tabular-nums shadow-xl">
                   <span className="uppercase text-sm tracking-[0.3em]">Net Cash Flow for the Period</span>
                   <span className="underline decoration-double decoration-2">৳ {formatCurrency(
                     Object.keys(periodBalances).reduce((sum, c) => {
                       const acc = activeCOA.find(a => a.code === c);
                       if (!acc) return sum;
                       const value = periodBalances[c] || 0;
                       return acc.balance === 'Credit' ? sum + value : sum - value;
                     }, 0)
                   )}</span>
                 </div>
              </div>
