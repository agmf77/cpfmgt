...
import { useFirestore, addDocumentNonBlocking, useCollection, useMemoFirebase, useDoc, updateDocumentNonBlocking, deleteDocumentNonBlocking, getDocsWithFallback, shouldUseLocalDatabase, readLocalCollection } from "@/firebase";
...
export default function NewTransactionPage() {
  ...
  const syncSubsidiaryLedgers = async (journalId: string, entryData: any) => {
    console.log('🔄 Syncing subsidiary ledgers for journal:', journalId);
      ...
      if (line.memberId) {
        ...
        if (editId) {
          ...
          if (shouldUseLocalDatabase()) {
            // For local database, query directly
            const localItems = await readLocalCollection(`members/${line.memberId}/fundSummaries`);
            existingDoc = localItems.find(item =>
              item.journalEntryId === editId &&
              item.accountCodeAtSource === line.accountCode
            );
          } else {
            // Use Firestore query
            ...
            if (!snapshot.empty) {
              existingDoc = { id: snapshot.docs[0].id };
            }
          }

          if (existingDoc) {
            updateDocumentNonBlocking(doc(firestore, "members", line.memberId, "fundSummaries", existingDoc.id), {
              ...summaryData,
    ...
    console.log('✅ Subsidiary ledger sync completed');
  };
      ...
      onConfirm: async () => {
        for (const line of existingTransaction.lines || []) {
          if (line.memberId) {
            ...
            const snapshot = await getDocsWithFallback(q);
            snapshot.forEach((d: any) => deleteDocumentNonBlocking(d.ref));
          }
        }
