...
export default function SpecialInterestDPPage() {
  ...
  const handleCalculate = async () => {
    ...
    for (const member of targetMembers) {
      ...
      const snapshot = await getDocsWithFallback(query(summariesRef, orderBy("summaryDate", "asc")));
      ...
      let runningBalance = allEntries.filter((e: any) => new Date(e.summaryDate) <= openingRefDate).reduce((sum: number, e: any) => sum + ((Number(e.employeeContribution)||0) - (Number(e.loanWithdrawal)||0) + (Number(e.loanRepayment)||0) + (Number(e.profitEmployee)||0) + (Number(e.profitLoan)||0) + (Number(e.pbsContribution)||0) + (Number(e.profitPbs)||0)), 0);
