      ...
      try {
        ...
        data.forEach((entry: any) => {
          ...
          const mapped = {
            ...
            chartOfAccountId: entry["chartOfAccountId"] || entry.chartOfAccountId || investmentAccounts[0]?.code || "101.10.0000",
              ...
              <DialogHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => {
                    const defaultAccountId = investmentAccounts[0]?.code || "101.10.0000";
        const templateData = [{ "Bank Name": "Sonali Bank PLC", "Ref No": "FDR-2024-001", "Principal": 1000000, "Initial Principal": 1000000, "Rate": 12.5, "First Opening Date": "2020-01-01", "Renew Date": "2024-01-01", "Maturity Date": "2025-01-01", "Instrument Type": "FDR", "chartOfAccountId": defaultAccountId, "Status": "Active" }];
          ...
          <Dialog open={isAddOpen} onOpenChange={(open) => { setIsAddOpen(open); if (!open) setEditingInvestment(null); }}>
            ...
            <DialogContent className="w-full max-w-5xl bg-white border-2 border-black">
              ...
              <form onSubmit={handleSaveInvestment} className="space-y-6 pt-4">
                <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                  ...
                  <div className="space-y-2"><Label className="text-[10px] font-black uppercase tracking-widest">Ref No</Label><Input name="referenceNumber" defaultValue={editingInvestment?.referenceNumber} className="h-10 border-2 border-black font-black" required /></div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input 
                      ...
                      className="h-10 border-2 border-black font-black tabular-nums" 
                      required 
                    />
                  </div>
                  <div className="space-y-2">
                  ...
                  <div className="space-y-2"><Label className="text-[10px] font-black uppercase tracking-widest">Yield Rate (%)</Label><Input name="interestRate" type="number" step="0.01" defaultValue={editingInvestment ? (editingInvestment.interestRate * 100).toFixed(2) : ""} onKeyDown={handleNumericKeyDown} className="h-10 border-2 border-black font-black tabular-nums" required /></div>
                  
                  <div className="col-span-2 grid grid-cols-1 gap-4 p-6 bg-slate-50 rounded-2xl border-2 border-black md:grid-cols-3">
                    <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select name="chartOfAccountId" defaultValue={editingInvestment?.chartOfAccountId || investmentAccounts[0]?.code || "101.10.0000"}>
                      <SelectTrigger className="h-10 border-2 border-black font-black"><SelectValue /></SelectTrigger>
                      <SelectContent>
                ...
                <DialogFooter className="gap-3 pt-4 justify-end"><Button type="button" variant="outline" className="h-10 border-2 border-black font-black px-6 uppercase" onClick={() => setIsAddOpen(false)}>Cancel</Button><Button type="submit" className="h-10 bg-black text-white font-black px-8 uppercase tracking-widest shadow-xl">Save Instrument</Button></DialogFooter>
              </form>
            </DialogContent>
          </Dialog>
        </div>
